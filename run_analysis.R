...
# Create the tidy data set as a data frame:
tidy_dataset<-as.data.frame(dataset)

# Create 2 arrays (for act and for subj) to store mean values:
tds_act <- array(data = 0, dim = length(u_std_names))
tds_subj <- array(data = 0, dim = length(u_std_names))

# Create the tidy data set:
for (i in 5:70){
    tds_act[i-4] <- summarize(group_by(tidy_dataset, act), mean(tidy_dataset[,i]))
    tds_subj[i-4] <- summarize(group_by(tidy_dataset, subj), mean(tidy_dataset[,i])) 
}
